document.addEventListener("DOMContentLoaded", () => {
    ...
    window.sendSms = function() {
        ...
        fetch(`${contextPath}/mvc/auth/undecided/verify.my?mode=send&phoneNumber=${phone}`)
            .then(res => res.text())
            .then(data => {
                if (data === "success") {
                    alert("인증번호가 발송되었습니다.");
    ...
    window.verifyCode = function() {
        ...
        // JSON 형태로 서버에 전달
        fetch(`${contextPath}/mvc/auth/undecided/verify.my?mode=check`, {
            method: 'POST',
    ...
    document.querySelector(".userModifyBtn").addEventListener("click", () => {
        ...
        // 메인 마이페이지로 이동
        location.href = `${contextPath}/mvc/auth/undecided/myPage.my`;
    });
});