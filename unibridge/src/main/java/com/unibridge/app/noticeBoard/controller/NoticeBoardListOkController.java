...
public class NoticeBoardListOkController implements Execute {

	@Override
	public Result execute(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		System.out.println("===noticeBoardListOkController 실행===");
...
//		System.out.println("페이징 확인 : " + pageMap);
		...
		// 게시글 목록 조회
		List<NoticeBoardListDTO> noticeBoardList = noticeBoardDAO.selectAll(pageMap);
		request.setAttribute("noticeBoardList", noticeBoardList);

		// 페이징 정보 설정
		// noticeBoardMapper.xml의 getTotal을 이용하여 전체 게시글 개수 조회
		// 실제 마지막 페이지 번호(realEndPage)를 계산함
		int total = noticeBoardDAO.getTotal();
