...
public class MentorVerifySubmitController implements Execute{
    ...
    public Result execute(HttpServletRequest request, HttpServletResponse response) 
        ...
        // 2. 비로그인 상태 접근 제어
        if (loginUser == null) {
        	//현재 jsp 이후에 mem으로 적용(미리적용)
            result.setPath(request.getContextPath() + "/mvc/auth/signin.mem"); // 로그인 페이지 경로
        ...
        if (isPwCorrect && isPhoneVerified != null && isPhoneVerified) {
            ...
            result.setPath(request.getContextPath() + "/mvc/auth/mentor/updateOk.my");
            result.setRedirect(true); 
        } else {
            // 검증 실패 시 에러 메시지 설정
            if (!isPwCorrect) {
            ...
            result.setPath("/app/user/mentor/myPage/userManage/userModifyCheck.jsp");
